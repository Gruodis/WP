...
class CheckoutView {
    ...
    public static function renderGuestsChooser($reservedRoom, $roomIndex, $roomType, $booking)
    {
        ...
        <p class="mphb-guest-name-wrapper hidden">
        ...
        if (count($allowedRates) > 0) {
			...
			$manodataX;
                foreach ($allowedRates as $rate) {
					...
					$manodata = $booking->getCheckInDate();
					$manodataX = $manodata;
				...
				<?php
if ($bigRatePrice < $ratePrice) {
  echo "a is bigger than b";
		$bigRate = $rate;
		$bigRatePrice = $ratePrice;
		$bigInputID = $inputId;
		$bigInputName = $inputName;
		$bigRateId = $rateId;
		$bigSelectedRateId = $selectedRateId;
...
<p class="mphb-room-rate-variant">
                        <label for="<?php echo esc_attr($inputId); ?>">
                            ...
                            <strong>
                                <?php echo $rate->getTitle() // . ', ' . $ratePrice; ?>
                            </strong>
                        </label>
                        <br>
                        <?php echo $rate->getDescription(); ?>
                    </p>
-->
                <?php } // For each allowed rate ?>
				<?php echo "FINALY" . $bigRatePrice; ?>
				<p class="mphb-room-rate-variant">
                        <label for="<?php echo esc_attr($bigInputID); ?>">
                            <input type="radio" id="<?php echo esc_attr($bigInputID); ?>" name="<?php echo esc_attr($bigInputName); ?>" class="mphb_sc_checkout-rate mphb_checkout-rate mphb-radio-label" value="<?php echo esc_attr($bigRateId); ?>" <?php checked($bigSelectedRateId, $bigRateId); ?> checked="checked">
                            <strong>
                                <?php echo $bigRate->getTitle() // . ', ' . $ratePrice; ?>
                            </strong>
                        </label>
                        <br>
                        <?php echo $bigRate->getDescription(); ?>
                    </p>
				<?php // echo "BUBUBUBUB" . \MPHB\Utils\DateUtils::formatDateWPFront( $booking->getCheckInDate() ); ?>
				<?php 
//			setlocale(LC_TIME,'lt_LT');
//$paymentDate = date("Y M d"); // Today
//			setlocale(LC_ALL, 'lt_LT');
//echo $paymentDate; // echos today! 
//$contractDateBegin =  \MPHB\Utils\DateUtils::formatDateWPFront( $booking->getCheckInDate() );
//$contractDateEnd  =  \MPHB\Utils\DateUtils::formatDateWPFront( $booking->getCheckOutDate() );
//
//if (
//  $paymentDate > $contractDateBegin && 
//  $paymentDate < $contractDateEnd){
//  echo "is between" . $contractDateBegin . "___" . $contractDateEnd;
//}else{
//   echo "NO GO! Begind date  " . $contractDateBegin . "___End date " . $contractDateEnd;  
//}
			?>
