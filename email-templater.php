<?php
// dir MHBP/includes/emails/templaters/email-templater.php
...
class EmailTemplater extends AbstractTemplater {
	...
	private function _fillBookingTags( &$tags ){
		$bookingTags	 = array(
			...
			array(
				'name'			 => 'time_to_expire',
				'description'	 => __( 'Time to Expire', 'motopress-hotel-booking' ),
			),
			// Customer
			array(
	...
	*/
	...
	public function replaceTag( $match ){
		...
		switch ( $tag ) {
			...
			// Time to Expire
			case 'time_to_expire':
				$replaceText = MPHB()->settings()->payment()->getPendingTime() . "val.";
				break;
