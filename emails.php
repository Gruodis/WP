...
/* ----------------------------------------------------------------
*
* MPHB>includes>emails>emails.php
*/
...
class Emails {
	...
	public function sendBookingConfirmedWithPaymentEmail( $booking, $payment ){
		...
		$this->getEmail( 'customer_approved_booking' )->trigger( $booking, array( 'payment' => $payment ) ); // EDITED !!!!!!!!!!!!!!!!!!!!!
	}
		...
		switch ( $booking->getStatus() ) {
			...
			case BookingCPT\Statuses::STATUS_CONFIRMED:
                if ( MPHB()->settings()->main()->getConfirmationMode() == 'payment' ) {
                    ...
                    if ( !is_null( $payment ) ) {
                        $this->getEmail( 'customer_confirmation_booking' )->trigger( $booking, array( 'payment' => $payment ) ); // EDITED !!!!!!!!!!!!!!!!!!!!!
                    } else {
                        $this->getEmail( 'customer_confirmation_booking' )->trigger( $booking ); // EDITED !!!!!!!!!!!!!!!!!!!!!
                    }
                } else {
                    $this->getEmail( 'customer_confirmation_booking' )->trigger( $booking ); // EDITED !!!!!!!!!!!!!!!!!!!!!
                }
	...
	public function resendConfirmationEmail( $booking ) {
		if( $booking->getStatus() == BookingCPT\Statuses::STATUS_CONFIRMED ) {
			if ( MPHB()->settings()->main()->getConfirmationMode() == 'payment' ) {
					...
					$payment = $expectPayment !== false ? MPHB()->getPaymentRepository()->findById( $expectPayment ) : null;
					if ( !is_null( $payment ) ) {
							$this->getEmail( 'customer_confirmation_booking' )->trigger( $booking, array( 'payment' => $payment ) );
					} else {
							$this->getEmail( 'customer_confirmation_booking' )->trigger( $booking );
					}
			} else {
